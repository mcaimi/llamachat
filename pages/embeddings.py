...
# build streamlit UI
st.set_page_config(
    page_title="🧠 Embeddings", initial_sidebar_state="collapsed", layout="wide"
)
...
if uploaded_files:
    ...
    if st.button("Convert And Embed Documents....", type="primary"):
        ...
        st.markdown(
            f"Successfully converted {len(converted_docs)} documents. Ready for ingestion..."
        )
        ...
        st.markdown(
            f"Successfully chunked: {len(rag_docs)} chunks to embed. Ready for embedding..."
        )
